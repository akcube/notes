...
rearr_less = np.loadtxt ('inp1.txt')
rearr_more = np.loadtxt ('inp2.txt')
#t = np.arange(0, 1e6, 1)
...
plt.plot(y2, rearr_less, 'r-', color=red, lw=2, label = "Rearranged to converge to 0.5512...")
#ax2 = plt.subplot(111);
#ax2.fill_between(y1, 0, original, alpha=.3)

y3 = np.arange(start=0, stop=rearr_more.shape[0]);
plt.plot(y3, rearr_more, 'g-', color=green, label = "Rearranged to converge to 0.8932...")
